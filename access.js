/* ===========================================================
   ...
   (This file used to be named gate.js. Renamed to access.js because a
   filename containing "gate" is a known false-positive trigger for some
   ad-blocker / privacy-extension filter lists, which silently blocked it
   from loading last time.)
...
(function () {
  ...
  var PASSWORD_B64 = "Y29sb21iaWFjb2Zlcm1lbnQ="; // "colombiacoferment"
  ...
  function reveal() {
    ...
    if (payload && content && !content.dataset.filled) {
      ...
      // reveal.js scans the DOM for [data-reveal] elements at the moment
      // it runs — those elements don't exist until the line above, so
      // load it fresh now rather than relying on a static <script> tag
      // that would have run (and found nothing) before unlock.
      loadScript("reveal.js");
    }
